...
def prepp_kjoretoy(inn_fil: Union[str, pathlib.Path]) -> pl.DataFrame:
    ...
    df.with_columns(pl.col("tekn_reg_f_g_n").cast(pl.Utf8).str.strptime(pl.Datetime,"%Y%m%d").alias("dato_første"),
    pl.col("tekn_neste_pikk").str.strptime(pl.Datetime,"%Y%m%d%", Strict=False).alias("dato_neste"),
    pl.col("tekn_reg_eier_dato").cast(pl.Utf8).str.strptime(pl.Datetime,"%Y%m%d").alias("dato_eier"),)
    ...
    # !!!! ERSTATT MED DIN KODE:
    df = df.with_columns(
        pl.col("tekn_drivstoff").cast(pl.Float64, strict=False)
    )
